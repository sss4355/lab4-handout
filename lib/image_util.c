/* CMSC 16200 - lab 4
 * File: image_util.c 
 * Implementations of utility functions on image processing
 */
...
void buffer_to_arrays(pixel pixels[ROWS][COLS]) {
    ...
    for (y=0; y<read_height; y++) {
        ...
        for (x=0; x<read_width; x++) {
            ...
            pixels[y][x] = make_pixel((int) ptr[3], (int) ptr[0], (int) ptr[1], (int) ptr[2]);
        }
    }
}
...
void arrays_to_buffer(pixel pixels[ROWS][COLS]) {
    ...
    for (y=0; y<read_height; y++) {
        png_byte* row = (png_bytep) malloc(4 * width * sizeof(png_byte));
        for (x=0; x<read_width; x++) {
            png_byte* ptr = &(row[x*4]);
            // printf("Pixel at position [ %d - %d ] has RGBA values: %d - %d - %d - %d\n",
            //       x, y, ptr[0], ptr[1], ptr[2], ptr[3]);
            ptr[0] = (png_byte) get_red(pixels[y][x]);
            ptr[1] = (png_byte) get_green(pixels[y][x]);
            ptr[2] = (png_byte) get_blue(pixels[y][x]);
            ptr[3] = (png_byte) get_alpha(pixels[y][x]);
        ...
        fprintf(fp,"<tr align=center style=\"height:15px\">\n");
            for(j=0;j<image_width;j++) {
                ...
                fprintf(fp," bgcolor=\"#%.2x%.2x%.2x",
                    get_red(pixels[i][j]),get_green(pixels[i][j]),get_blue(pixels[i][j])); //TODO
                fprintf(fp,"\">&nbsp;</td>\n");
            }
        fprintf(fp,"</tr>\n");
    }
